...
{
    char* str = &word[start];   /*сслыка на адрес 1го байта нового слова*/
    str[end - start] = '\0';    /*добавка знака окончания строки, чтобы не тянулся мусор*/
    if (str[0] == NULL)         /*пустые строки, если 2 пробела или зарезервированные символы идут подряд*/
    {
        return;
    }
    if (CheckOnInt(str) == 1)
